...
double MeshGraph::AverageDistanceBetweenVertices() const {
    ...
    for (int i = 0; i < adjList.size(); i++) {
        ...
        for (it = adjList[i].begin(); it != adjList[i].end(); ++it) {
            result = result + Double3::Distance(vertices[base].position3D,
                                                (*it)->position3D);
            counter++;
        }
    }
...
int MeshGraph::TotalVertexCount() const { return vertices.size(); }
...
void MeshGraph::PaintInBetweenVertex(std::vector<Color> &outputColorAllVertex,
    ...
    if ((vertexIdFrom >= vertices.size() or vertexIdFrom < 0) or
        (vertexIdTo >= vertices.size() or vertexIdTo < 0)) {
        ...
        while (my_heap.HeapSize() > 0) {
            ...
            if (visited[current_vertex_id]) {
                ...
                for (std::list<Vertex *>::const_iterator it =
                         adjList[current_vertex_id].begin();
                     it != adjList[current_vertex_id].end(); it++) {
                    double dist = distances[current_vertex_id] +
                                  Double3::Distance(
                                      vertices[current_vertex_id].position3D,
                                      (*it)->position3D);
    ...
    if (vertexId >= vertices.size() or vertexId < 0) {
        ...
        while (my_min_heap.HeapSize() > 0) {
            ...
            if (visited[curr_vertex]) {
                ...
                for (std::list<Vertex *>::const_iterator it =
                         adjList[curr_vertex].begin();
                     it != adjList[curr_vertex].end(); it++) {
                ...
                for (int k = 0; k < sorted.size(); k++) {
                    ...
                    double dist =
                        ...
                        Double3::Distance(vertices[curr_vertex].position3D,
                                          vertices[curr].position3D);
                    ...
                    if (distances[curr] == INFINITY) {
                        ...
                        if (type == FILTER_GAUSSIAN) {
                            filtered_version =
                                std::exp(-(pow(dist, 2) / pow(alpha, 2)));
                        }
        ...
        for (int j = 0; j < vertices.size(); j++) {
            if (all_depths[j] < maxDepth + 1 and all_depths[j] != -1) {
                ...
                filtered_color.r =
                    (unsigned char)(((double)color.r) * filter_coef[j]);
                filtered_color.g =
                    (unsigned char)(((double)color.g) * filter_coef[j]);
                filtered_color.b =
                    (unsigned char)(((double)color.b) * filter_coef[j]);
    ...
    if (vertexId >= vertices.size() or vertexId < 0) {
        ...
        while (my_min_heap.HeapSize() > 0) {
            ...
            if (visited[curr_vertex]) {
                ...
                for (int k = 0; k < sorted.size(); k++) {
                    curr = sorted[k];
                    double dist = Double3::Distance(vertices[base].position3D,
                                                    vertices[curr].position3D);
                    double filtered_version;
